...
| Web API | 已提供 `/api/state`、`/api/chat`、`/api/projects`、项目级 reset、SSE 事件推送、工作区文件读取/写入接口 |
| 前端工作台 | 已具备项目侧栏、工件查看/编辑、Agent 对话、SSE 增量消息、工具进度事件、图片上传与项目级操作 |
...
| 测试 | 已完成测试收口与合并；当前后端全量结果为 `93 passed` |
