...
- **核心目标**：让开发者可以通过 Ludens-Flow 配置、测试并导出面向游戏运行时的大模型能力，逐步从“开发期辅助”延伸到“游戏内可用能力”。
- **近期重点**：
  - 梳理游戏内 LLM 的最小闭环：NPC 对话、任务提示、剧情回顾、世界观问答等典型场景。
  - 完善工作台内的 Agent / Prompt / Tool 组合配置模型。
  - 设计运行时配置导出格式，包括模型路由、Prompt、上下文边界、调用参数与权限声明。
  - 优先验证 Unity 接入骨架：服务端接口约定、客户端调用示例、错误与超时处理。
  - 加入成本控制、调用频率限制、上下文裁剪和运行时安全边界设计。
- **进度状态**：前端配置与导出页面壳已完成，下一阶段推进真实运行时接入方案设计与最小闭环验证。
---
