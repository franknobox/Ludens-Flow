...
class DesignAgent(BaseAgent):
    ...
    def discuss(
        ...
        base_prompt_text = (
            ...
            "5. Level Layout Studio / 关卡设计台能力归属于 Dam：当用户讨论关卡、空间动线、房间布局、地形节奏或导入导出的布局结果时，你负责给出设计解释、布局建议和体验取舍；不要把它误当成工程执行或模型配置能力。\n"
            "6. 语气保持有想法、有协作感，但不要空泛。\n"
        )
    ...
    def commit(
        ...
        prompt_text = (
            ...
            "3. 在关卡 / 内容结构部分，如果项目涉及空间布局、房间连接、地形节奏或关卡草图，请给出可在 Level Layout Studio / 关卡设计台继续细化的布局建议。\n"
            "4. 在 MVP 边界部分，明确区分必须完成的核心体验和可以后置的想法。\n"
            "5. 不要编造尚未确定的细节。如果讨论中确实没有决定，请标记为“【待确认：...】”。"
            "但已经讨论过的内容必须如实保留。\n"
            "6. 保持 Markdown 清晰可读，可以少量使用强调，但不要加入额外包装话术。\n"
            "7. 末尾增加两个简短部分：\n"
