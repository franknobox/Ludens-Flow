"""
文件功能：review_agent 角色 Agent 实现，负责对应阶段的讨论与提交逻辑。
...
class ReviewAgent(BaseAgent):
    ...
    def commit(
        ...
        prompt = (
            "请严格审视以下三份核心文档，并根据项目目标引擎 / 工作区配置评估一致性、可行性和 scope 健康度；未配置目标引擎时，使用通用小型游戏 demo 开发视角，不要默认 Unity：\n\n"
            f"{engine_context}\n\n"
            f"【GDD】\n{gdd}\n\n【PROJECT PLAN】\n{pm}\n\n【IMPLEMENTATION PLAN】\n{impl}\n\n"
            "1. 请在正文区域输出一份专业的 Markdown 评审报告，包含以下维度：\n"
            "   - 创意与设计评审：核心玩法是否清晰，游戏体验是否自洽，有何亮点与潜在问题。\n"
            "   - 工程可执行性审视：IMPLEMENTATION_PLAN 中描述的架构是否适合当前目标引擎或通用 demo 环境；对于当前规模是否过度设计或遗漏关键技术点。\n"
