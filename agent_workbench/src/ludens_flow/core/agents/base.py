...
from ludens_flow.capabilities.tools.registry import dispatch_tool_call, merge_tool_schemas
        ...
        while (
            ...
            for tool_call in response.tool_calls:
                ...
                try:
                    ...
                    error_text = str(exc) or "Tool execution failed."
                    if not error_text.startswith("["):
                        error_text = f"[TOOL_ERROR:EXECUTION_FAILED] {error_text}"
                    if tool_event_handler:
                        tool_event_handler(
                            {
                                "type": "tool_failed",
