...
class ReviewAgent(BaseAgent):
    ...
    def discuss(
        ...
        return AgentResult(
            assistant_message="Review Agent is in DISCUSS mode. Usually we jump direct to COMMIT."
        )
    ...
    def commit(
        ...
        logger.info(
            f"[ReviewAgent] Commit generated, Gate status resolved to: {status}"
        )
        ...
        scores = (
            gate_dict.get("scores", {})
            if isinstance(gate_dict.get("scores", {}), dict)
            else {}
        )
    ...
    def _parse_json_gate(self, report_text: str) -> tuple[Dict[str, Any], str]:
        ...
        if match:
            try:
                ...
                clean_md = (
                    report_text[: match.start()].strip()
                    + "\n\n"
                    + report_text[match.end() :].strip()
                )
            except Exception as e:
                logger.error(
                    f"[ReviewAgent] JSON blocks captured but failed to parse: {e}"
                )
                gate_data = None
        else:
            logger.error(
                "[ReviewAgent] Could not find <<REVIEW_GATE_JSON>> markers in output."
            )
        ...
        has_block_issue = any(
            issue.get("severity", "").upper() == "BLOCK"
